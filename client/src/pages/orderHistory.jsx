  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const fetchedOrders = res.data.order_delivery[0]?.order_carts || [];
        ...
        {orders.map((order) => (
          <div key={order._id} className="bg-white p-8 rounded-2xl shadow-xl mb-8 transform transition duration-500 hover:scale-105">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className={`text-2xl ${order.cart_status === 'completed' ? 'text-green-500' : 'text-red-500'}`}>
                {order.cart_status === 'completed' ? <FaCheckCircle /> : <FaTimesCircle />} {order.cart_status}
              </span>
            </div>
            <div className="border-t-2 border-gray-200 pt-4">
              <p className="text-gray-700"><strong>Date:</strong> {new Date(order.createdAt).toLocaleDateString()}</p>
              <p className="text-gray-700"><strong>Total:</strong> ${order.cart_total}</p>
            </div>
